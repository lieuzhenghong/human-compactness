...
def _get_best_and_worst_plans_2(grouped_df, cutoff):
    grouped_df.loc[grouped_df["hc"] > grouped_df["hc"].quantile(cutoff), "from"] = "hc"
    top_hc_plans = grouped_df[
        grouped_df["hc"] > grouped_df["hc"].quantile(cutoff)
    ].copy()
    grouped_df.loc[grouped_df["pp"] > grouped_df["pp"].quantile(cutoff), "from"] = "pp"
    top_pp_plans = grouped_df[
        grouped_df["pp"] > grouped_df["pp"].quantile(cutoff)
    ].copy()
    grouped_df.loc[grouped_df["ch"] > grouped_df["ch"].quantile(cutoff), "from"] = "ch"
    top_ch_plans = grouped_df[
        grouped_df["ch"] > grouped_df["ch"].quantile(cutoff)
    ].copy()
    grouped_df.loc[
        grouped_df["reock"] > grouped_df["reock"].quantile(cutoff), "from"
    ] = "reock"
    top_reock_plans = grouped_df[
        grouped_df["reock"] > grouped_df["reock"].quantile(cutoff)
    ].copy()
    grouped_top_df = [top_hc_plans, top_pp_plans, top_ch_plans, top_reock_plans]
...
def _get_best_and_worst_plans(grouped_df):
    ...
    for cutoff in cutoffs:
        ...
        grouped_df.loc[
            grouped_df["pp"] > grouped_df["pp"].quantile(cutoff), "from"
        ] = "pp"
        top_pp_plans = grouped_df[
            grouped_df["pp"] > grouped_df["pp"].quantile(cutoff)
        ].copy()
        grouped_df.loc[
            grouped_df["ch"] > grouped_df["ch"].quantile(cutoff), "from"
        ] = "ch"
        top_ch_plans = grouped_df[
            grouped_df["ch"] > grouped_df["ch"].quantile(cutoff)
        ].copy()
...
def _get_lowest_sd_plans(grouped_df):
    cutoff = 0.05
    grouped_df.loc[grouped_df["sd"] < grouped_df["sd"].quantile(cutoff), "from"] = "sd"
    return grouped_df[grouped_df["sd"] < grouped_df["sd"].quantile(cutoff)].copy()


def _plot_point_plot(gdf):
    theme_set(theme_classic())
    reg_plot = (
        ggplot(gdf, aes(x="hc", y="sd"))
    ...
    reg_plot = (
        ggplot(gdf, aes(x="hc", y="sd", color="factor(state)"))
        + geom_point()
        + geom_smooth()
    )
    reg_plot.save("./30_results/individual_regressions.png", height=20, width=20)
...
for idx, top_df in enumerate(top_dfs):
    ...
    for i in range(4):
        ...
        diff_mean = top_df[i][["sd"]].mean() - grouped_dfs[idx][["sd"]].mean()
        ...
        # print(values)
        x = ttest_ind(top_df[i][["sd"]], grouped_dfs[idx][["sd"]], equal_var=False)
        # print(x)
...
for metric in ["hc", "pp", "reock", "ch"]:
    fit = ols(f"sd ~ {metric} + C(state) -1", data=gdf).fit()
...
for idx, metric in enumerate(["hc", "pp", "reock", "ch"]):
    fit = ols(f"sd ~ {metric} + C(state) - 1", data=top_plans[idx]).fit()
    print(fit.summary())
    print(fit.summary().as_latex())
...
print(f"Mean SD of top plans under Human Compactness: {top_plans[0][['sd']].mean()}")
print(f"Mean SD of top plans under Polsby-Popper: {top_plans[1][['sd']].mean()}")
