...
def build_dataframe_from_list(data_list, start_from):
    ...
    for plan_id, step in enumerate(data_list):
        for (district_id, district) in enumerate(step["human_compactness"]):
            ...
            data_dict["sd"].append(step["spatial_diversity"][1][str(district_id)])
            data_dict["hc"].append(step["human_compactness"][str(district_id)])
            data_dict["pp"].append(step["polsby_compactness"][str(district_id)])

            if "reock_compactness" in step:
                data_dict["reock"].append(step["reock_compactness"][str(district_id)])
            else:
...
def build_initial_district_assignment(state_code, path, census_tracts_df):
    ...
    assert len(starting_dict["id"]) == len(starting_dict["district_assignment"])

    print(
        min(starting_dict["district_assignment"]),
...
def read_shapefile(state_code, path):
    ...
    ctdf = gpd.read_file(f"{path}/Tract2000_{state_code}.shp")
    ...
    for idx, step in enumerate(assignment_list):
        ...
        for district_id, area in grouped.items():
            if district_id == -1:
                ...
                # remember 1-indexing
                df.loc[
                    (df["plan"].eq(idx + 1) & df["district"].eq(district_id + 1)),
                    "area",
                ] = area
                df.loc[
                    (df["plan"].eq(idx + 1) & df["district"].eq(district_id + 1)),
...
def plot_vrps_on_census_tracts(census_tracts, STATE_CODE):
    ...
    vrps = sample_rvps.sample_rvps(CDB, STATE_CODE, DEM_RVPS, REP_RVPS, SAMPLE_SIZE)

    census_tracts = census_tracts.to_crs("EPSG:2163")

    vrps = vrps.to_crs("EPSG:2163")

    ax0 = census_tracts.plot(column="district_assignment", alpha=0.6, cmap="tab20")
...
def build_and_save_df_to_csv(STATE_CODE, NUM_DISTRICTS, SHAPEFILE_PATH, SAVE_PATH):
    ...
    files = [(f"./20_intermediate_files/{STATE_CODE}/data{x}.json", x) for x in s]

    print("Reading points...")
    df = pd.concat(
        [
            build_dataframe_from_list(read_results_from_file(f[0]), f[1] - 100)
            for f in files
        ]
    )
    ...
    try:
        ...
        print(f"Could not find {LOAD_PATH}/{STATE_CODE}_df.csv, generating one now...")
...
def _plot_corr_matrix(df):
    ...
    df["area_binned"] = pd.cut(
        df["log_area"], bins=[0, 21, 23, 99], labels=["small", "medium", "large"]
    )
    print(df)

    g = sns.pairplot(
        df[["sd", "hc", "pp", "reock", "ch", "area_binned"]],
        hue="area_binned",
        palette="Set2",
    )
    ...
    sns.heatmap(corr, cmap="Reds", linewidths=0.5, square=True, annot=True, ax=ax)
...
def _plot_corr_matrix_grouped(df):
    ...
    g = sns.pairplot(df[["sd", "hc", "pp", "reock", "ch"]], palette="Set2")
...
def _plot_top_plans(grouped_df):
    ...
    for cutoff in cutoffs:
        grouped_df.loc[
            grouped_df["hc"] > grouped_df["hc"].quantile(cutoff), "from"
        ] = "hc"
        top_hc_plans = grouped_df[
            grouped_df["hc"] > grouped_df["hc"].quantile(cutoff)
        ].copy()
        grouped_df.loc[grouped_df["pp"] > grouped_df["pp"].quantile(0.9), "from"] = "pp"
        top_pp_plans = grouped_df[
            grouped_df["pp"] > grouped_df["pp"].quantile(cutoff)
        ].copy()
        grouped_df.loc[grouped_df["ch"] > grouped_df["ch"].quantile(0.9), "from"] = "ch"
        top_ch_plans = grouped_df[
            grouped_df["ch"] > grouped_df["ch"].quantile(cutoff)
        ].copy()
        grouped_df.loc[
            grouped_df["reock"] > grouped_df["reock"].quantile(0.9), "from"
        ] = "reock"
        top_reock_plans = grouped_df[
            grouped_df["reock"] > grouped_df["reock"].quantile(cutoff)
        ].copy()
        ...
        means_boxplot = (
            ggplot(grouped_top_df, aes(x="from", y="sd", fill="from"))
            + geom_boxplot()
            + geom_hline(aes(yintercept=grouped_df[["sd"]].mean(), linetype=["mean"]))
        )
        means_boxplot.save(f"{SAVE_PATH}/{STATE_CODE}_means_boxplot_{cutoff}.png")
        ...
        means_violin.save(f"{SAVE_PATH}/{STATE_CODE}_means_violin {cutoff}.png")

        means_boxplot = (
            ggplot(grouped_top_df_with_mean, aes(x="from", y="sd", fill="from"))
            + geom_violin(draw_quantiles=0.5)
            + geom_hline(aes(yintercept=grouped_df[["sd"]].mean(), linetype=["mean"]))
        )
        means_boxplot.save(
        ...
        hc_corrplot = ggplot(top_hc_plans, aes(x="hc", y="sd")) + geom_point(
            color="cornflowerblue", alpha=0.5, size=0.5
        )
        hc_corrplot.save(f"{SAVE_PATH}/{STATE_CODE}_top_hc_sd_corrplot {cutoff}.png")

        ch_corrplot = ggplot(top_ch_plans, aes(x="ch", y="sd")) + geom_point(
            color="cornflowerblue", alpha=0.5, size=0.5
        )
        ch_corrplot.save(f"{SAVE_PATH}/{STATE_CODE}_top_ch_sd_corrplot {cutoff}.png")
...
def _squarerootify(df):
    df[["sd", "hc", "pp", "reock", "ch"]] = df[["sd", "hc", "pp", "reock", "ch"]] ** 0.5
    return df
...
if __name__ == "__main__":
    ...
    for STATE_CODE in num_districts:
        ...
        for SAVE_PATH in SAVE_PATHS:
            ...
            fig.savefig(f"{SAVE_PATH}/{STATE_CODE}_hc_sd_log_area.png", dpi=100)
            ...
            print(grouped_df[["hc_pp_delta", "pp", "hc", "sd", "reock", "ch"]].corr())
